...
const positionMode = (memory, index) => memory[index] === undefined ? 0 : memory[index]
const immediateMode = (_memory, index) => index;
const relativeMode = (memory, index) => memory[index] +relativeBase;
...
const value = (mode, memory, index) => {
  ...
  return memory[address] === undefined ? 0 : memory[address];
};
...
export const intcode = (input, systemInput) => {
  ...
  while (memory[instructionPointer] !== 99) {
    ...
    if (opcode === 9) {
      const ofset = value(mode1, memory, instructionPointer + 1);
      relativeBase += ofset;
      instructionPointer += 2;
      continue;
    }
